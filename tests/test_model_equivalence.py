...
@pytest.mark.slow
def test_v1_1_nano_model_with_weights_equivalence() -> None:
    """Test loading nano model without weights."""
    opm_model = opm.load_model_from_id(
        opm.ModelID.OLMOEARTH_V1_1_NANO, load_weights=True
    )
    assert opm_model is not None
    opm_param_count = sum(p.numel() for p in opm_model.parameters())
    assert opm_param_count > 0
    ...
    # now lets do the same for an olmoearth_pretrain model
    op_model = op_loader.load_model_from_id(
        op_loader.ModelID.OLMOEARTH_V1_1_NANO, load_weights=True
    )
    assert op_model is not None
    op_param_count = sum(p.numel() for p in op_model.parameters())
    assert op_param_count == opm_param_count
...
@pytest.mark.slow
def test_v1_2_nano_model_with_weights_equivalence() -> None:
    """Test loading nano model without weights."""
    opm_model = opm.load_model_from_id(
        opm.ModelID.OLMOEARTH_V1_2_NANO, load_weights=True
    )
    assert opm_model is not None
    opm_param_count = sum(p.numel() for p in opm_model.parameters())
    assert opm_param_count > 0
    ...
    # now lets do the same for an olmoearth_pretrain model
    op_model = op_loader.load_model_from_id(
        op_loader.ModelID.OLMOEARTH_V1_2_NANO, load_weights=True
    )
    assert op_model is not None
    op_param_count = sum(p.numel() for p in op_model.parameters())
    assert op_param_count == opm_param_count
